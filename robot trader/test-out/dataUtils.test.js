...
(0, node_test_1.describe)('TseApiClient', () => {
    ...
    (0, node_test_1.test)('fetchOrderBook fetches real data successfully', async () => {
        ...
        node_assert_1.default.deepStrictEqual(data.bids, mockOrderBookData.bids);
        node_assert_1.default.deepStrictEqual(data.asks, mockOrderBookData.asks);
    ...
    (0, node_test_1.test)('fetchOrderBook falls back to twin on fetch failure', async () => {
        ...
        const client = new dataUtils_1.TseApiClient(config);
        const data = await client.fetchOrderBook('TEST');
        node_assert_1.default.strictEqual(data.bids.length, 50);
        node_assert_1.default.strictEqual(data.asks.length, 50);
        node_assert_1.default.ok(data.queueDynamics.buyVolume > 0);
    });
    ...
    (0, node_test_1.test)('fetchMarketData throws error if proxyUrl is not configured', async () => {
        const config = {
            proxyUrl: '',
            apiKey: 'key',
            isConnected: true,
        ...
        const client = new dataUtils_1.TseApiClient(config);
        await node_assert_1.default.rejects(async () => await client.fetchMarketData('TEST'), /API proxy URL is not configured/);
    });
    (0, node_test_1.test)('fetchAdvancedMetrics throws error if proxyUrl is not configured', async () => {
        const config = {
            proxyUrl: '',
            apiKey: 'key',
            isConnected: true,
        ...
        const client = new dataUtils_1.TseApiClient(config);
        await node_assert_1.default.rejects(async () => await client.fetchAdvancedMetrics([]), /API proxy URL is not configured/);
    });
    ...
    (0, node_test_1.test)('fetchMultiTimeframeData falls back to full simulation on API fetch failure', async () => {
        const config = {
            proxyUrl: 'http://proxy.com',
            apiKey: 'key',
            isConnected: true,
        ...
        try {
            ...
            node_assert_1.default.strictEqual(warningLogged, true, `Expected warning to be logged, but got: ${loggedWarning}`);
        }
        finally {
            console.warn = originalConsoleWarn;
        }
    ...
    (0, node_test_1.test)('fetchMultiTimeframeData falls back to full simulation when API fetch fails', async () => {
        const originalConsoleWarn = console.warn;
        ...
        try {
            ...
            const data = await client.fetchMultiTimeframeData('TEST');
            node_assert_1.default.ok(Array.isArray(data['1d']));
            node_assert_1.default.ok(Array.isArray(data['1h']));
            node_assert_1.default.ok(Array.isArray(data['15m']));
            node_assert_1.default.ok(Array.isArray(data['1m']));
            node_assert_1.default.strictEqual(warnLogged, true, 'console.warn should have been called');
        }
        finally {
            console.warn = originalConsoleWarn;
        }
