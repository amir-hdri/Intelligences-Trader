...
class TseApiClient {
    ...
    async fetchOrderBook(symbolId) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (json.bids && json.asks) {
                        const buyVolume = json.bids.reduce((sum, item) => sum + item.quantity, 0);
                        const sellVolume = json.asks.reduce((sum, item) => sum + item.quantity, 0);
        ...
        const bids = new Array(LEVELS);
        const asks = new Array(LEVELS);
...
const analyzeMarketMTF = (mtfData, symbolId = "", externalMetrics, weights = optimizedWeights, precalc) => {
    ...
    const rsi = precalc?.rsi ?? (0, exports.calculateRSI)(hPrices);
    const macd = precalc?.macd ?? (0, exports.calculateMACD)(hPrices);
    const atr = precalc?.atr ?? (0, exports.calculateATR)(hourlyCandles);
    const bb = precalc?.bb ?? (0, exports.calculateBollingerBands)(hPrices);
    const ichimoku = precalc?.ichimoku ?? (0, exports.calculateIchimoku)(hourlyCandles);
    const regime = precalc?.regime ?? (0, exports.detectMarketRegime)(hourlyCandles, atr);
    let score = 0;
    const reasons = [];
    // 1. Trend Analysis
    ...
    for (let i = 1; i < hPrices.length; i++) {
        if (hPrices[i - 1] > 0)
            logReturns.push(Math.log(hPrices[i] / hPrices[i - 1]));
    }
    ...
    return {
        ...
        indicators: {
            ...
            bollinger: {
                upper: bb.upper,
                lower: bb.lower,
                middle: bb.middle ?? bb.mid ?? (bb.upper + bb.lower) / 2
            },
            ichimoku: {
...
const optimizeStrategyWeights = (candles) => {
    ...
    for (let j = 50; j < candles.length - 1; j++) {
        ...
        for (let i = 0; i < 15; i++) {
            const forecast = (0, exports.analyzeMarketMTF)(mtfData, "", undefined, candidates[i], precalc);
            if (forecast.action !== "HOLD") {
                const profit = forecast.action === "BUY"
