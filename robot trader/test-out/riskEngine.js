...
class RiskEngine {
    ...
    updateEquity(newEquity, usedMargin = 0) {
    ...
    updatePerformanceMetrics(winRate, profitFactor) {
    ...
    isIranianHoliday(date) {
        const dayOfWeek = date.getDay();
        // Thursday (4) and Friday (5) are weekends in Iran
        if (dayOfWeek === 4 || dayOfWeek === 5) {
            return true;
        }
        const { jm, jd } = jalaali_js_1.default.toJalaali(date);
        // Fixed solar Hijri holidays
        if (jm === 1 && jd >= 1 && jd <= 4)
            return true; // Nowruz
        if (jm === 1 && jd === 12)
            return true; // Islamic Republic Day
        if (jm === 1 && jd === 13)
            return true; // Sizdah Bedar
        if (jm === 3 && jd === 14)
            return true; // Khomeini's Death
        if (jm === 3 && jd === 15)
            return true; // Khordad 15 Revolt
        if (jm === 11 && jd === 22)
            return true; // Revolution Day
        if (jm === 12 && jd === 29)
            return true; // Oil Nationalization Day
        return false;
    }
    validateTrade(forecast, activeTrades, symbolInfo, advancedRisk) {
    ...
    calculateKellySize(price, atr, suggestedRiskCapitalPct) {
    ...
    calculateTrailingStop(currentPrice, entryPrice, action, atr) {
    ...
    calculateCommission(volume, type) {
    ...
    estimateSlippage(orderBookPressure, regime, volume) {
        // Base slippage
        let slippageBps = 2;
