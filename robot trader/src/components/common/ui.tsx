...
export const Badge:React.FC<{tone?:BadgeTone; children:React.ReactNode}> = ({tone='slate',children}) => {
  const map:Record<BadgeTone,string>={violet:"bg-violet-500/10 text-violet-300 border-violet-500/20",emerald:"bg-emerald-500/10 text-emerald-400 border-emerald-500/20",red:"bg-red-500/10 text-red-400 border-red-500/20",amber:"bg-amber-500/10 text-amber-300 border-amber-500/20",slate:"bg-white/5 text-[#94A3B8] border-white/10"};
...
export const Alert:React.FC<{tone?:AlertTone; children:React.ReactNode}> = ({tone='amber',children})=>{
  const m:Record<AlertTone,string>={amber:"bg-amber-500/10 border-amber-500/20 text-amber-300",red:"bg-red-500/10 border-red-500/20 text-red-300",violet:"bg-violet-500/10 border-violet-500/20 text-violet-300"};
