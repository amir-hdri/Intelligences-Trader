...
export const MetricCard:React.FC<{title:string;value:string;sub?:string;icon:LucideIcon;delta?:{v:number;pos:boolean};accent?:string;loading?:boolean; tooltip?:string}> = ({title,value,sub,icon:Icon,delta,accent,loading,tooltip})=>{
 ...
 return <div title={tooltip} tabIndex={0} className="glass-card rounded-2xl p-4 flex flex-col gap-3 hover:border-white/10 transition-colors focus-within:ring-2 focus-within:ring-blue-500/30">
  <div className="flex items-center justify-between"><span className="text-[11px] tracking-[0.06em] font-bold text-[#64748B] uppercase">{title}</span><Icon className={cn("w-4 h-4",accent||"text-[#94A3B8]")} /></div>
