...
import { BrainCircuit, CheckCircle, XCircle, Clock, Target, Zap, Activity } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="glass-panel p-6 rounded-3xl relative overflow-hidden group">
          ...
          <h3 className="text-slate-500 font-black text-[11px] uppercase tracking-[0.06em] mb-4">Neural Learning Context</h3>
          <div className="flex items-end gap-3 mb-6">
             <span className="text-5xl font-black text-white tracking-tighter text-glow">{completed.length}</span>
             <span className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-wider">Signals Resolved</span>
          </div>
          <div className="flex gap-2">
             <span className="px-2.5 py-1 bg-emerald-500/10 text-emerald-400 text-[11px] font-black rounded-lg border border-emerald-500/20 shadow-sm">{wins} WINS</span>
             <span className="px-2.5 py-1 bg-rose-500/10 text-rose-400 text-[11px] font-black rounded-lg border border-rose-500/20 shadow-sm">{completed.length - wins} LOSSES</span>
          </div>
        </div>

        <div className="glass-panel p-6 rounded-3xl relative overflow-hidden group">
          <h3 className="text-slate-500 font-black text-[11px] uppercase tracking-[0.06em] mb-4">Verification Accuracy</h3>
          <div className="flex items-end gap-3 mb-6">
             <span className={`text-5xl font-black tracking-tighter text-glow ${winRate >= 50 ? 'text-emerald-400' : 'text-rose-400'}`}>
               {winRate.toFixed(1)}%
             </span>
             <span className="text-[11px] font-black text-slate-500 mb-2 uppercase tracking-wider">Hedge Ratio</span>
          </div>
        ...
        <div className="glass-panel p-6 rounded-3xl relative overflow-hidden group col-span-1 sm:col-span-2 lg:col-span-1">
           <h3 className="text-slate-500 font-black text-[11px] uppercase tracking-[0.06em] mb-4">Neural Strategy Bias</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-panel p-6 rounded-3xl h-full flex flex-col">
           <h3 className="text-slate-400 font-black text-[11px] uppercase tracking-[0.06em] mb-8 flex items-center gap-2">
             ...
             {weightChanges.map((w) => (
               <div key={w.key} className="group glass-card p-4 rounded-2xl transition-all border-slate-800/40 hover:scale-[1.02]">
                 <div className="flex justify-between items-center mb-3">
                    <span className="text-[11px] font-black uppercase tracking-wider text-slate-300">{w.key}</span>
                    <div className={`px-2 py-0.5 rounded text-[11px] font-mono font-black border ${w.diff > 0.1 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : w.diff < -0.1 ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-slate-800/50 text-slate-500 border-slate-700/50'}`}>
                      {w.current.toFixed(2)}
                    </div>
                 </div>
                 <div className="relative h-1.5 bg-slate-900/50 rounded-full overflow-hidden border border-slate-800/50">
                    <div className="absolute top-0 bottom-0 w-0.5 bg-blue-500/50 z-10" style={{ left: `${(w.default / 5) * 100}%` }} />
                    <div
           ...
           <div className="mt-8 p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
              <div className="flex items-center gap-2 text-[11px] font-black text-blue-400 uppercase tracking-[0.06em] mb-2">
        ...
        <div className="lg:col-span-2 glass-panel rounded-3xl overflow-hidden flex flex-col">
           <div className="p-6 border-b border-slate-800/50 flex justify-between items-center bg-slate-900/40">
             <h3 className="text-slate-400 font-black text-[11px] uppercase tracking-[0.06em] flex items-center gap-2">
               <Target className="w-3 h-3 text-blue-400" />
               Signal Verification Log
             </h3>
             <span className="text-[11px] font-black text-slate-500 uppercase tracking-wider">Rolling Window: 50 Signals</span>
           </div>

           <div className="overflow-y-auto max-h-[650px] scrollbar-thin scrollbar-thumb-slate-800/50">
             <table className="w-full text-left text-xs">
               <thead className="bg-slate-900/60 text-slate-500 font-black uppercase tracking-[0.06em] sticky top-0 backdrop-blur-xl z-10 border-b border-slate-800/50">
                 <tr>
               ...
               <tbody className="divide-y divide-slate-800/40">
                   ...
                   <tr key={p.id} className="hover:bg-white/[0.02] transition-colors group">
                     ...
                     <td className="px-6 py-4">
                       <div className="flex flex-col gap-1">
                         ...
                         <span className="text-[11px] text-slate-500 font-bold uppercase tracking-tight group-hover:text-slate-400 transition-colors">{p.reason.split('.')[0]}</span>
                       </div>
                     </td>
                     ...
                     <td className="px-6 py-4 text-center">
                       {p.status === 'WIN' && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 text-emerald-400 rounded-lg text-[11px] font-black border border-emerald-500/20 shadow-sm"><CheckCircle size={10} /> WIN</span>}
                       {p.status === 'LOSS' && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-500/10 text-rose-400 rounded-lg text-[11px] font-black border border-rose-500/20 shadow-sm"><XCircle size={10} /> LOSS</span>}
                       {p.status === 'PENDING' && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-500/10 text-blue-400 rounded-lg text-[11px] font-black border border-blue-500/20 shadow-sm animate-pulse"><Clock size={10} /> ACTIVE</span>}
                     </td>
                 ...
                 {history.length === 0 && (
                   <tr>
                     <td colSpan={5} className="px-6 py-32 text-center text-slate-600 font-black uppercase tracking-[0.06em] italic opacity-40">
